...
class GANModule(nn.Module):
    def __init__(self, 
        ...
        self.label_embedding_shape = label_embedding_shape # [n_labels, n_neurons]
        self.n_labels = n_labels
        self.label_embedding = nn.Embedding(self.label_embedding_shape[0], self.label_embedding_shape[1])
...
class DNNGenerator(GANModule):
    def __init__(self, 
        ...
        self.input_size = self.latent_noise_size + self.label_embedding_shape[1]
    ...
    def forward(self, noise, labels):
        ...
        gen_input = torch.cat((noise, self.label_embedding(labels.to(torch.int).flatten())), -1)
...
class DNNDiscriminator(GANModule):
    def __init__(self, 
        ...
        self.input_size = self.data_shape[0] * self.data_shape[1] + self.label_embedding_shape[1]
        self.model = nn.Sequential(
    ...
    def forward(self, data, labels):
        ...
        d_input = torch.cat((data, self.label_embedding(labels.to(torch.int).flatten())), -1)
