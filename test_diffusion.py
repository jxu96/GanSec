...
def main(args):
    ...
    if os.path.exists('backups/diffusion'):
        model.load_state_dict(torch.load('backups/diffusion/model.pth', weights_only=True, map_location=torch.device('cpu')))
    else:
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)

        epochs = 5
        for epoch in range(epochs):
            for imgs, _ in A_loader:
